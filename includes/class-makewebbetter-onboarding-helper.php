...
class Makewebbetter_Onboarding_Helper {
	...
	public function __construct() {
		...
		add_action( 'admin_head', array( $this, 'add_onboarding_popup_screen' ) );
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', MWB_REFUND_N_EXCHANGE_LITE_URL . 'admin/js/makewebbetter-onboarding-admin.js', array( 'jquery' ), '1.0.0', true );
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once MWB_REFUND_N_EXCHANGE_LITE_DIRPATH . 'includes/extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			rand() => array(
				'id' => 'monthly-revenue',
				'label' => esc_html__( 'What is your monthly revenue?', 'woo-refund-and-exchange-lite' ),
				'type' => 'radio',
			...
			rand() => array(
				'id' => 'industry_type',
				'label' => esc_html__( 'What industry defines your business?', 'woo-refund-and-exchange-lite' ),
				'type' => 'select',
				'name' => 'industry_type_',
				'value' => '',
				'multiple' => 'yes',
			...
			rand() => array(
				'id' => 'onboard-email',
				'label' => esc_html__( 'What is the best email address to contact you?', 'woo-refund-and-exchange-lite' ),
				'type' => 'email',
				'name' => 'email',
			...
			rand() => array(
				'id' => 'onboard-number',
				'label' => esc_html__( 'What is your contact number?', 'woo-refund-and-exchange-lite' ),
				'type' => 'text',
				'name' => 'phone',
				'value' => '',
				'required' => 'yes',
			...
			case 'select2':
				...
				if ( ! empty( $options ) && is_array( $options ) ) {
					...
					<select class="on-boarding-select-field <?php echo esc_html( $select2 ); ?> <?php echo esc_html( $class ); ?>" id="<?php echo esc_html( $id ); ?>" name="<?php echo esc_html( $name ); ?>[]" <?php echo esc_html( $required ); ?> <?php echo esc_html( $is_multiple ); ?>>
						...
						<?php if ( 'select' == $type ) : ?>	
							<option class="on-boarding-options" value=""><?php esc_html( 'Select Any One Option...', 'woo-refund-and-exchange-lite' ); ?></option>
						<?php endif; ?>
