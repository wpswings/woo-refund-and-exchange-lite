...
class woocommerce_refund_and_exchange_lite_Public {
	...
	// print_r(wp_get_current_user()->caps);die;
		...
		$translation_array = array(
			...
			'return_subject_msg' => __( 'Please enter refund subject.', 'woocommerce-refund-and-exchange-lite' ),
			'return_reason_msg'	=> __( 'Please enter refund reason.', 'woocommerce-refund-and-exchange-lite' ),
			);
		...
		if ( $check_ajax ) 
		{
			if(current_user_can('ced-rnx-refund-request'))
			{
				if(isset($_FILES['ced_rnx_return_request_files']))
				{
					...
					{
						$filename = array();
						$order_id = sanitize_text_field($_POST['ced_rnx_return_request_order']);
						...
						for($i=0;$i<$count;$i++)
						{
							...
							{	
								...
								$sourcePath = sanitize_text_field($_FILES['ced_rnx_return_request_files']['tmp_name'][$i]);
								$targetPath = $directory.'/'.$order_id.'-'.sanitize_text_field($_FILES['ced_rnx_return_request_files']['name'][$i]);

								$filename[] = $order_id.'-'.sanitize_text_field($_FILES['ced_rnx_return_request_files']['name'][$i]);
	...
	function  ced_rnx_order_return_button($order)
	{
		...
		if($ced_rnx_return == 'yes')
		{
			...
			{
				?>
				<h2><?php _e( 'Refund Requested Product', 'woocommerce-refund-and-exchange-lite' ); ?></h2>
				<?php 
				...
				foreach($product_datas as $key=>$product_data)
				{
					...
					<table class="shop_table order_details">
						...
						<tbody>
							...
							?>
							<tr>
								<th scope="row"><?php _e('Refund Amount', 'woocommerce-refund-and-exchange-lite') ?></th>
							...
							if(isset($added_fees))
							{
								if(is_array($added_fees))
								{
									foreach($added_fees as $da=>$added_fee)
									{
										if(!empty($added_fee))
										{
											if($da == $key)
											{
												?>
												<tr>
													<th colspan="2"><?php _e('Extra Cost', 'woocommerce-refund-and-exchange-lite') ?></th>
												</tr>
												<?php 
												foreach($added_fee as $fee)
												{
													...
													<tr>
														...
														<td><?php echo wc_price($fee['val']);?></td>
													</tr>
													<?php 
									...
									?>
									<tr>
										<th scope="row"><?php _e('Total Refund Amount', 'woocommerce-refund-and-exchange-lite') ?></th>
										<th><?php echo wc_price($product_data['amount']); ?></th>
									</tr>
									<?php 
								}
					...
					if(in_array($order_status, $statuses))
					{
						if($product_data['status'] == 'pending')
						{
								...
								if($day_allowed >= $day_diff && $day_allowed != 0)	
								{
									if($ced_rnx_return_button_show)
									{
										...
										<form action="<?php echo $return_url ?>" method="post">
											...
											<p>
												<input type="submit" class="btn button" value="<?php _e('Update Request','woocommerce-refund-and-exchange-lite');?>" name="ced_update_return_request">
											</p>
										</form>
										<?php 
									}
							...
							if($product_data['status'] == 'complete')
							{
								...
								?>
								<p><?php _e('Above product Refund request is approved on','woocommerce-refund-and-exchange-lite');?> <b><?php echo $appdate?>.</b></p>
								<?php 
							}
							...
							if($product_data['status'] == 'cancel')
							{
								...
								$appdate=date_format($appdate,$format);
								?>
								<p><?php _e('Above product Refund request is cancelled on','woocommerce-refund-and-exchange-lite');?> <b><?php echo $appdate?>.</b></p>
								<?php 
							}
						}
